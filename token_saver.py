...
def convert_with_markitdown(input_path: Path) -> str:
    ...
    if not hasattr(converter, "convert_local"):
        raise RuntimeError(
            ...
            "Upgrade MarkItDown before using token_saver.py."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output Markdown file. Defaults to <input-stem>.token-saver.md",
    )
    parser.add_argument(
        "--save-raw-md",
        type=Path,
...
def resolve_markdown_output(input_path: Path, output_path: Path | None) -> Path:
    if output_path is None:
        resolved = input_path.with_name(f"{input_path.stem}.token-saver.md")
    else:
